...
static inline int8_t nonnegative_int_encode(ndnup_buffer_write_t *out, uint32_t value)
{
    if (value > 0x0000FFFF) {
        ndnup_buffer_write(out, (value & 0xFF000000) >> 24);
        ndnup_buffer_write(out, (value & 0x00FF0000) >> 16);
    }
    if (value > 0x000000FF) {
        ndnup_buffer_write(out, (value & 0x0000FF00) >> 8);
    }
    ndnup_buffer_write(out, (value & 0x000000FF) >> 0);
    return 0;
}
